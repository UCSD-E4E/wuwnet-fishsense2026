...
def test_bricks_on_the_targets_vertical_edge_are_recovered_as_two_faces():
    ...
    Dropping those loses the marker bricks specifically, because the markers sit
    at the wall ends -- which is where the edges are -- and correspondence is
    seeded from markers.
    """
    from fishsense_wuwnet.calibration_model import MARKER_COLOURS

    _, quads, correspondence = _detect((260.0, -330.0, 190.0))
    assert sum(q.colour in MARKER_COLOURS for q in quads) >= 6
    assert sum(f.colour in MARKER_COLOURS for f in correspondence.faces) >= 8
...
def test_the_three_dimensional_target_settles_the_focal_length_ratio():
    ...
    assert ratio == pytest.approx(truth[0, 0] / truth[1, 1], rel=2e-3)
    assert fitted[0, 0] == pytest.approx(truth[0, 0], rel=5e-3)
