...
| §6 | `pool-decoy-length-by-camera-model` | final | 12 of 13 decoy frames; 1 rejected for a mask whose aspect ratio misses the calipered 2.95 by more than a quarter. Companion to §4.2's range figure by the opposite route: there the range comes from a known object's pose and no laser, here from the laser beam and the decoy's size never enters it. Left: the uncorrected model ranges 25.7 % short of the two corrected ones, which differ by 0.6 %. Right: all three read the same length, median -2.6 / -2.8 / -2.7 % in legend order. Length cannot see the error that range shows plainly. |
